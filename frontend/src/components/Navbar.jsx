...
import { Activity, LogOut, LayoutDashboard, PlusCircle, User, Settings, Database, ChevronDown, Globe, Menu, X } from 'lucide-react';
            ...
            <nav className="glass" style={{
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    ...
                    <div className={`nav-links ${mobileMenuOpen ? 'mobile-open' : ''}`}>
                        <Link to="/" onClick={() => setMobileMenuOpen(false)} style={{ textDecoration: 'none', color: 'var(--text-main)', display: 'flex', alignItems: 'center', gap: '0.4rem', fontWeight: 500 }}>
                            <LayoutDashboard size={18} /> Dashboard
                        </Link>
                        <Link to="/input" onClick={() => setMobileMenuOpen(false)} style={{ textDecoration: 'none', color: 'var(--text-main)', display: 'flex', alignItems: 'center', gap: '0.4rem', fontWeight: 500 }}>
                        ...
                        <div style={{ position: 'relative' }} ref={dropdownRef}>
                            ...
                            {dropdownOpen && (
                                <div style={{
                                    ...
                                    border: '1px solid var(--border)', overflow: 'hidden', padding: '0.5rem', zIndex: 1000
                                }}>
                                    ...
                                    <button
                                        onClick={() => { setShowProfileModal(true); setDropdownOpen(false); setMobileMenuOpen(false); }}
                                    ...
                                    <button
                                        onClick={() => { setShowHealthModal(true); setDropdownOpen(false); setMobileMenuOpen(false); }}
                                        className="dropdown-item"
                                        style={{ width: '100%', padding: '0.6rem', display: 'flex', alignItems: 'center', gap: '0.5rem', background: 'none', border: 'none', cursor: 'pointer', borderRadius: '6px', textAlign: 'left', fontSize: '0.9rem', color: 'var(--text-main)' }}
                                    >
